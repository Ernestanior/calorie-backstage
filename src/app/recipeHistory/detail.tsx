...
const RecipeHistoryDetail: FC<IProps> = ({ visible, data, onCancel }) => {
  if (!data) return null;

  const formatDate = (date: string | undefined) => {
    if (!date) return '-';
    // 处理 "2025-11-24 15:37:51" 格式
  ...
  const baseItems: DescriptionsProps['items'] = [
    ...
    { label: '菜系', children: data.cuisine || '-' },
    { label: '创建日期', children: formatDate(data.createDate) },
    { label: '创建者', children: data.createBy || '-' },
    { label: '修改日期', children: formatDate(data.modifyDate) },
    { label: '修改者', children: data.modifyBy || '-' },
  ];
  ...
  return (
    ...
    >
      ...
      {responseDto && (responseDto.name || responseDto.imageUrl || responseDto.locale) && (
        <div style={{ marginTop: 24 }}>
          <Collapse defaultActiveKey={['1']}>
            <Panel header="生成的食谱详情" key="1">
              <Descriptions 
                items={[
                  { label: '名称', children: responseDto.name || '-' },
                  { label: '菜系', children: responseDto.cuisine || '-' },
                  { 
                    label: '图片', 
                    children: responseDto.imageUrl ? (
                      <Image
                        src={ responseDto.imageUrl}
                        alt={responseDto.name || data.mealName || '食谱图片'}
                        width={200}
                        height={200}
                  ...
                  { 
                    label: '食材', 
                    children: responseDto.ingredients?.length ? (
                      <Space wrap>
                        {responseDto.ingredients.map((item: string, index: number) => (
                          <Tag key={index}>{item}</Tag>
                        ))}
                      </Space>
                    ) : '-'
                  },
                  { label: '烹饪时间（分钟）', children: responseDto.cookingTime !== undefined && responseDto.cookingTime !== null ? responseDto.cookingTime : '-' },
                  { label: '难度', children: responseDto.difficulty || '-' },
                  { 
                    label: '提示', 
                    children: responseDto.tips?.length ? (
                      <Space direction="vertical" size="small">
                        {responseDto.tips.map((tip: string, index: number) => (
              ...
              {responseDto.steps?.length > 0 && (
                <div style={{ marginTop: 24 }}>
                  <h3 style={{ marginBottom: 16 }}>制作步骤</h3>
                  {responseDto.steps.map((step: any, index: number) => (
                    <div key={index} style={{ marginBottom: 16, padding: 16, border: '1px solid #f0f0f0', borderRadius: 8 }}>
                      <div style={{ fontWeight: 600, marginBottom: 8 }}>步骤 {step.step || index + 1}</div>
                      <div style={{ marginBottom: 8 }}>{step.description || '-'}</div>
                      {step.time && <div>时间：{step.time} 分钟</div>}
                      {step.temperature && <div>温度：{step.temperature}</div>}
                    </div>
                  ))}
                </div>
              )}
...
export default RecipeHistoryDetail;
