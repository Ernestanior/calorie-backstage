...
let index = 0
const RecipeSetCreate: FC<IProps> = ({ visible, onRefresh, onCancel }) => {
    const [form] = useForm()
    ...
    const onSubmit = async () => {
        ...
        if (!name || !nameEn || !day || !hot || !weight || !type || !color || !visible || !labelList.length || !image.length) {
            setErrMsg('* 请填写完整')
            return
        } else {
            setErrMsg('')
        ...
        const params = {
            day, hot, type, weight, name, nameEn, color, visible,
    ...
    return (
        <Drawer title="新增食谱" width={900} onClose={onCancel} open={visible}>
            <Form form={form} initialValues={{ visible: 1 }}>
...
export default RecipeSetCreate;
